...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const scheduleDefaultReminders = () => {
    ...
    if (settings.morningReminder) {
      scheduleRecurringReminder('morning', 9, 0, 'Good morning! 🌅 Ready to start your calorie quest today?')
    }

    // Evening reminder (7 PM)
    if (settings.eveningReminder) {
      scheduleRecurringReminder('evening', 19, 0, 'Evening check-in! 🌙 How did your quest go today?')
    }
  ...
  const scheduleStreakReminder = () => {
    ...
    const checkStreak = () => {
      ...
      if (lastActivity) {
        ...
        if (hoursAgo >= 24) {
          showNotification('⚡ Don\'t break your streak! Log your meals to keep your quest alive!')
        }
      }
    }
  ...
  const showNotification = (message: string, icon?: string) => {
    ...
    const notification = new Notification('Calorie Quest', {
      body: message,
      icon: icon || '/favicon.svg',
      badge: '/favicon.svg',
      tag: 'calorie-quest-reminder',
